...
CREATE TABLE IF NOT EXISTS `pizza_projeto`.`tb_telefone` (
  ...
  `id_funcionario` INT NOT NULL,
  `_id_cliente` INT NOT NULL,
  PRIMARY KEY (`idt_telefone`),
  INDEX `fk_tb_telefone_tb_funcionario1_idx` (`id_funcionario` ASC) VISIBLE,
  INDEX `fk_tb_telefone_1_idx` (`_id_cliente` ASC) VISIBLE,
  CONSTRAINT `fk_tb_telefone_tb_funcionario1`
    FOREIGN KEY (`id_funcionario`)
    REFERENCES `pizza_projeto`.`tb_funcionario` (`id_funcionario`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_tb_telefone_1`
